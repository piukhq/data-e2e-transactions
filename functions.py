...
def atlas(df, merchant):
    ...
    if merchant == 'harvey-nichols': response = 'audit_data->\'response\'->\'body\'->\'CustomerClaimTransactionResponse\'->>\'outcome\' atlas_response'
    if merchant == 'wasabi-club': response = 'audit_data->\'response\'->\'body\'->>\'Message\' atlas_response'
    ...
    df = pd.merge(left=df,right=atlas,left_on='Transaction ID',right_on='transaction_id',how='left').drop(['transaction_id'],axis=1)
...
def matched(df):
    ...
    df = pd.merge(left=df,right=matched_tr,left_on='Transaction ID',right_on='transaction_id',how='left').drop(['transaction_id'],axis=1)
...
def tlog(df,merchant):
    ...
    if merchant == 'wasabi-club':
        ...
        where = f'(to_date(data->>\'Date\',\'DD/MM/YYYY\') >= \'{min_date}\' AND to_date(data->>\'Date\',\'DD/MM/YYYY\') <= \'{max_date}\')'
    df_tlog_query = f"""
            SELECT
                transaction_id,
    ...
    df = pd.merge(left=df,right=df_tlog,left_on='Transaction ID',right_on='transaction_id',how='left').drop(['transaction_id'],axis=1)
...
def wasabi(df):
    ...
    df = df[['Select your ID', 'Card Type', 'Payment Method',
       'Receipt number - AS PRINTED ON RECEIPT\nPlease consult the annotated receipts you were provided with separately to locate the receipt number.',
       'Store *\nPlease input the store address as found on the receipt. Consult the annotated receipts you were provided with separately to locate the transaction ID.',
       'Payment amount - AS PRINTED ON RECEIPT\nEnter only the number, including decimals (no £ sign).',
       'Date - AS PRINTED ON RECEIPT', 'Time - AS PRINTED ON RECEIPT']]
    df.columns = ['Tester ID','Card Type','Payment Method','Transaction ID','Store','Payment amount','Date', 'Time']
    ...
    df = pd.merge(left=df,right=mids,how='left',left_on=['Store','Card Type'],right_on=['Data Management','Payment Scheme']).drop(['Data Management', 'Payment Scheme'],axis=1)
    df = df[['Transaction ID','Card Type','Payment Method','Store','MID','Payment amount','Date', 'Time','Tester ID']]

    return df

def hniceland(df):
    df = df[pd.to_datetime(df['Completion time']).dt.isocalendar().week == (pd.Timestamp.today().week - 1)]
    df.columns = ['ID', 'Start time', 'Completion time', 'Email', 'Name', 'OccupierID', 
        'Tester ID', 'Please upload a photo of your receipt', 'Merchant', 'Card Type', 
        'Payment Method', 'MID', 'Payment amount', 'Date', 'Time', 'Auth code', 'Transaction ID']
    df = df[['Transaction ID', 'Merchant', 'Card Type',  'Payment Method', 'MID', 'Payment amount', 
        'Date', 'Time', 'Auth code', 'Tester ID']]
...
def iceland(df):
    ...
    df = pd.merge(df,tr_df,how='left',left_on=['Transaction ID','Card Type','Date','Payment amount','MID'],
